...
export default function Dashboard() {
  ...
  return (
    <PageShell eyebrow="Overview" title="Storage dashboard" description="Monitor storage, uploads, Telegram health and delivery activity across your workspaces.">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 sm:mt-4 sm:gap-4 xl:grid-cols-3">
      ...
      <div className="mt-4 grid gap-4 sm:mt-5 xl:grid-cols-[1.2fr_0.8fr] xl:gap-5">
        ...
        <Card className="min-w-0 p-3 sm:p-5">
          ...
          {data?.fileTypes.length ? (
            <div className="space-y-3">
              {data.fileTypes.map((type) => <div key={type._id} className="flex min-w-0 items-center justify-between gap-3 rounded-md bg-panel-2 p-3 text-sm"><span className="min-w-0 truncate text-white">{type._id}</span><span className="shrink-0 text-muted">{formatBytes(type.bytes)}</span></div>)}
            </div>
