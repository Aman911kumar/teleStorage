          ...
          <motion.div initial={{ opacity: 0, scale: 0.98, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }} className="surface mx-auto mt-24 max-w-xl overflow-hidden rounded-xl shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <Command>
              ...
              <Command.List className="thin-scrollbar max-h-80 overflow-auto p-2">
                ...
                {actions.map((action) => (
                  <Command.Item key={action.path} value={action.label} onSelect={() => { navigate(action.path); setOpen(false); }} className="flex cursor-pointer items-center justify-between rounded-md px-3 py-2.5 text-sm text-slate-200 transition data-[selected=true]:bg-white/[0.075] data-[selected=true]:text-white">
